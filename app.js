...
async function fetchTypesFor(name){
  ...
  const data = await res.json();
  const types = data.types.map(t=>capitalize(t.type.name)); // EN
...
function logLineKR(atkName, moveName, defName, range, eff, isCrit){
  ...
  const critLine = critText(isCrit);
  const head = `${atkName}의 ${moveName}!`;
  const tail = `${defName}에게 ${range[0]} ~ ${range[1]}의 데미지!`;
  return [head, effLine, critLine, tail].filter(Boolean).join("\n");
}

// ---- UI 바인딩 ----
function el(id){ return document.getElementById(id); }
function pct(x){ return (x*100).toFixed(1)+"%"; }
function showTypesBadges(containerId, typesEN){
  const box = el(containerId);
  box.innerHTML = typesEN.map(t=>`<span class="type">${TYPE_EN_TO_KO[t]||t}</span>`).join("");
}
...
function renderResult({range, oneHit, twoHit}, atk, def, moveName, eff, crit){
  ...
  el("outOHKO").innerHTML = `${pct(oneHit)}`;
  el("out2HKO").innerHTML = `${pct(twoHit)}`;
...
function setup(){
  attachSearch(el("atkSearch"), (name, typesEN)=>{
    ...
    showTypesBadges("atkBadge", typesEN);
  });
  attachSearch(el("defSearch"), (name, typesEN)=>{
    ...
    showTypesBadges("defBadge", typesEN);
  });
  ...
  el("btnSwap").addEventListener('click', ()=>{
    ...
    // 배지도 스왑
    const atkTypesEN = [toENType(el("atkType1").value), toENType(el("atkType2").value)].filter(Boolean);
    const defTypesEN = [toENType(el("defType1").value), toENType(el("defType2").value)].filter(Boolean);
    showTypesBadges("atkBadge", atkTypesEN);
    showTypesBadges("defBadge", defTypesEN);
  });
  ...
  el("btnCalc").addEventListener('click', ()=>{
    ...
    const attack = atk.stats.A;
    const defense = def.stats.B;
    ...
    const {damages, eff, crit} = computeDamage({
      ...
      opts:{
        isCrit, stabModeKO, reflect, lightscreen, isDouble, isBurn, hasGuts, weatherKO,
  ...
  // 초기 배지 표시
  showTypesBadges("atkBadge", []);
  showTypesBadges("defBadge", []);

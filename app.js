...
function attachSearch(inputEl, onPick){
  let timer;
  inputEl.addEventListener('change', onTry);
  inputEl.addEventListener('keydown', (e)=>{ if(e.key==='Enter') onTry(); });
  function onTry(){
    clearTimeout(timer);
    ...
    timer = setTimeout(async ()=>{
      try{
        const typesEN = await fetchTypesFor(q);
        onPick(q, typesEN);
        autoCalcIfReady();
      }catch{}
    }, 120);
...
function attachMoveAuto(){
  ...
  async function onMove(){
    const mv = elName.value.trim().toLowerCase().replace(/\s+/g,'-');
    if(!mv) return;
    try{
      const info = await fetchMoveDetail(mv);
      el("moveType").value = TYPE_EN_TO_KO[info.typeEN] || info.typeEN;
      el("moveCat").value = info.categoryKO==="상태" ? "물리" : info.categoryKO; // 상태는 계산 불가 → 기본 물리

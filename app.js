...
// ---- type maps, chart, helpers (same as v6) ----
const TYPE_KO_TO_EN = {
...
const cache = { list:null, moveList:null, ko2enPoke:{}, en2koPoke:{}, ko2enMove:{}, en2koMove:{} };
  ...
  if(/[가-힣]/.test(input)){
    ...
    for(const item of list){
      try{
        ...
        if(koName && (strip(koName)===strip(ko))){
          cache.ko2enPoke[ko] = item.name;
          cache.en2koPoke[item.name] = koName;
          persistMaps();
          return {en:item.name, ko:koName};
        }
      }catch{}
    }
  ...
  if(/[가-힣]/.test(raw)){
    ...
    for(const item of list){
      try{
        ...
        if(koName && (strip(koName)===strip(raw))){
          cache.ko2enMove[raw] = det.name;
          cache.en2koMove[det.name] = koName;
          persistMaps();
          return {en: det.name, ko: koName};
        }
...
function screenMultiplier(categoryKO){ return 1.0; } // reflect/lightscreen removed
...
function computeDamage({level, power, attack, defense, categoryKO, moveTypeEN, atkTypesEN, defTypesEN, opts}){
  ...
  const scr  = screenMultiplier(categoryKO);
